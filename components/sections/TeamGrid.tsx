        ...
        <Stagger className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {team.map((member) => (
            <AnimatedReveal key={member.name} className="h-full">
              <div className="h-full rounded-2xl border border-gray-200 bg-off-white p-6 text-center">
                ...
                <h3 className="mt-4 text-base font-medium text-black">{member.name}</h3>
                <p className="text-sm text-accent">{member.role}</p>
                <p className="mt-2 text-sm leading-relaxed text-gray-600">{member.bio}</p>
                <a
                  href={member.linkedin}
                  aria-label={`${member.name} on LinkedIn`}
                  className="mt-4 inline-flex h-8 w-8 items-center justify-center rounded-full border border-gray-200 text-gray-500 transition-colors hover:border-accent hover:text-accent"
                >
